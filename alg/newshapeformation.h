#ifndef NEWSHAPEFORMATION_H
#define NEWSHAPEFORMATION_H
#include "core/amoebotparticle.h"
#include "core/amoebotsystem.h"


class NewShapeFormationParticle : public AmoebotParticle {
public:
    enum class State {
    ...
    // State::Idle).
    NewShapeFormationParticle(const Node head, AmoebotSystem& system,
                           const State state, const QString mode);
    ...
    NewShapeFormationParticle& nbrAtLabel(int label) const;
    ...
    int nextShapeDir(int orientation) ;
...
protected:
    ...
    int _shapeDir;  // Corresponds to "dir" in paper.
    int turnSignal;

private:
    friend class NewShapeFormationSystem;
};


class NewShapeFormationSystem : public AmoebotSystem {
public:
    ...
    NewShapeFormationSystem(int numParticles = 200, double holeProb = 0.2,
                         QString mode = "h");
